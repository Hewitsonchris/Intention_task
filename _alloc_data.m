...
function data = _alloc_data(n_trials)
    ...
    % each frame
    MAX_FRAMES_PER_TRIAL = 240 * 60; % 60 seconds
    ...
    trials = struct('is_manipulated', z, ...
                    'manipulation_angle', z, ...
                    'online_feedback', z, ...
                    'endpoint_feedback', z, ...
                    'was_restarted', z, ...
                    'is_judged', z, ...
                    'judgement_data', struct('reaction_time', z, 'decision_time', z, 'loc', z, 'width', z, 'score', z), ...
                    'label', z, ... % see trial_labels.m for meaning (or otherwise encoded at block-level in data)
                    'too_slow', z); % TODO: pair ID?
    trials.target(1:MAX_TRIALS) = struct('x', 0, 'y', 0, 'angle', 0);
