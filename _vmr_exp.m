...
function _vmr_exp(is_debug, is_short, block_type, settings)
    ...
    tgt = make_tgt(settings.id, settings.sign, block_type, is_debug, is_short);
    ...
    while (beginning_state = sm.get_state())
        ...
        if (vbl_time - sm.trial_start_time) > 50
            was_restarted = true;
        ...
        if sm.will_be_new_trial()
            ...
            [loc, width, rt, dt, score, av_clamp] = sm.get_trial_data(trial_count);
            data.trials.judgement_data.reaction_time(trial_count) = rt;
            data.trials.judgement_data.decision_time(trial_count) = dt;
            data.trials.judgement_data.loc(trial_count) = loc;
            data.trials.judgement_data.width(trial_count) = width;
            data.trials.judgement_data.score(trial_count) = score;
            data.trials.judgement_data.av_clamp(trial_count) = av_clamp;
            was_restarted = false;
