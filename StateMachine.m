    ...
    methods
        ...
        function update(sm, evts, last_vbl)
		    ...
		    %%% CHANGE ALPHA? OR REMOVE ALTOGETHER?%%%
                ...
                if point_in_circle([sm.mouse.x sm.mouse.y], [sm.center.x sm.center.y], ...
                    ...
                    if trial.is_judged && ~sm.have_judged
                        sm.judge.loc = trial.target.angle; % TODO: fudge factor, this should be derived from elsewhere
        ...
        function draw(sm)
            ...
            if sm.judge.vis
                ...
                cd = cosd(sm.judge.loc);
                sd = sind(sm.judge.loc);
            ...
            if sm.instruct.vis
                DrawFormattedText(w.w, sm.instruct.text, 'center', 1.2 * w.center(2) + 125, 255);
            end
        ...
        function [loc, width, rt, dt, score, av_clamp] = get_trial_data(sm, trial_count)
            score = sm.score.val;
            trial = sm.tgt.trial(trial_count);
            av_clamp = sm.av_history;
            if trial.is_judged
                loc = sm.judge.loc;
